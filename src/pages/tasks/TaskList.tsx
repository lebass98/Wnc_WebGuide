...
const TaskList: React.FC = () => {
  ...
  const [newTask, setNewTask] = useState({
    title: '', dueDate: '', status: 'todo' as Task['status'], tag: '마케팅', assignee: 'Mayad Ahmed', description: ''
  });

  const toggleTask = (id: string) => setTasks(prev => prev.map(task => task.id === id ? { ...task, completed: !task.completed } : task));
  const handleDragStart = (id: string) => setDraggedTaskId(id);
  const handleDrop = (status: Task['status']) => { if (!draggedTaskId) return; setTasks(prev => prev.map(task => task.id === draggedTaskId ? { ...task, status } : task)); setDraggedTaskId(null); };
  ...
  const handleCreateTask = () => {
    ...
    setNewTask({ title: '', dueDate: '', status: 'todo', tag: '마케팅', assignee: 'Mayad Ahmed', description: '' });
  };
  ...
  const getTaskCount = (status: 'all' | 'todo' | 'inprogress' | 'completed') => status === 'all' ? tasks.length : tasks.filter(t => t.status === status).length;

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-[26px] font-bold text-slate-900 dark:text-white leading-tight">{t('tasks.title')}</h1>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#1A222C] w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden">
            <div className="p-8 pb-0 flex justify-between items-start">
              <div>
                <h3 className="text-2xl font-bold text-slate-800 dark:text-white">새 작업 추가하기</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">할 일 목록을 효율적으로 관리하세요</p>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 dark:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <CustomDatePicker placeholder="마감일 선택" onChange={(date) => { if (date instanceof Date) setNewTask({ ...newTask, dueDate: date.toLocaleDateString('ko-KR', { month: 'short', day: 'numeric', year: 'numeric' }) }); }} />
                </div>
                <div className="space-y-2">
...
interface TaskGroupProps { title: string; status: Task['status']; count: number; tasks: Task[]; onToggle: (id: string) => void; onDragStart: (id: string) => void; onDragOver: (e: React.DragEvent) => void; onDrop: (status: Task['status']) => void; }

const TaskGroup: React.FC<TaskGroupProps> = ({ title, count, tasks, onToggle, onDragStart, onDragOver, onDrop }) => (
  <div onDragOver={onDragOver} onDrop={() => onDrop('todo' as any)} className="space-y-4 min-h-[100px]">
    <div className="flex items-center justify-between"><div className="flex items-center gap-3"><h3 className="text-lg font-bold text-slate-800 dark:text-white">{title}</h3><span className="px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-xs font-bold text-slate-500 dark:text-slate-400">{count}</span></div><button className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"><MoreHorizontal className="w-5 h-5" /></button></div>
    <div className="flex flex-col gap-3">{tasks.map(task => <TaskItem key={task.id} task={task} onToggle={onToggle} onDragStart={onDragStart} />)}</div>
  </div>
);
...
const TaskItem: React.FC<{ task: Task; onToggle: (id: string) => void; onDragStart: (id: string) => void }> = ({ task, onToggle, onDragStart }) => (
  <div draggable onDragStart={() => onDragStart(task.id)} className="group flex items-center gap-4 bg-white dark:bg-[#1A222C] p-4 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm hover:border-indigo-300 dark:hover:border-indigo-500/50 transition-all cursor-move active:opacity-50">
    ...
    <div className="hidden sm:flex items-center gap-6 shrink-0 ml-4">
      {task.tag && <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${task.tag.color}`}>{task.tag.name}</span>}
      {task.dueDate && <div className="flex items-center gap-1.5 text-slate-400 dark:text-slate-500"><Calendar className="w-4 h-4" /><span className="text-xs font-medium">{task.dueDate}</span></div>}
      {(task.comments !== undefined || task.attachments !== undefined) && <div className="flex items-center gap-3">{task.comments !== undefined && <div className="flex items-center gap-1.5 text-slate-400 dark:text-slate-500"><MessageSquare className="w-4 h-4" /><span className="text-xs font-medium">{task.comments}</span></div>}{task.attachments !== undefined && <div className="flex items-center gap-1.5 text-slate-400 dark:text-slate-500"><Paperclip className="w-4 h-4" /><span className="text-xs font-medium">{task.attachments}</span></div>}</div>}
      <img src={task.userAvatar} alt="Avatar" className="w-7 h-7 rounded-full border-2 border-white dark:border-slate-800 object-cover" />
    </div>
  </div>
);
