...
const SignUpPage: React.FC = () => {
    ...
    return (
        <div className="flex justify-center min-h-screen bg-white font-sans max-h-screen overflow-hidden dark:bg-slate-900 transition-colors duration-300">
            <button onClick={toggleTheme} className="fixed bottom-6 right-6 w-12 h-12 bg-[#16232A] hover:bg-[#23343e] dark:bg-[#FF5B04] dark:hover:bg-[#e04f00] rounded-full flex items-center justify-center text-white shadow-lg transition-colors z-50">
                ...
                <div className="w-full max-w-[420px] mx-auto mb-auto mt-16">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1.5"><label className="text-[13px] font-bold text-slate-700 dark:text-slate-300 block">{t('auth.name')}<span className="text-red-500">*</span></label><input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="이름" className="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-1 focus:ring-[#16232A] focus:border-[#16232A] dark:focus:ring-[#FF5B04] dark:focus:border-[#FF5B04] bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-sm transition-all" /></div>
                            <div className="space-y-1.5"><label className="text-[13px] font-bold text-slate-700 dark:text-slate-300 block">성<span className="text-red-500">*</span></label><input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="성" className="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-1 focus:ring-[#16232A] focus:border-[#16232A] dark:focus:ring-[#FF5B04] dark:focus:border-[#FF5B04] bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-sm transition-all" /></div>
                        </div>
                        <div className="space-y-1.5"><label className="text-[13px] font-bold text-slate-700 dark:text-slate-300 block">{t('auth.email')}<span className="text-red-500">*</span></label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="이메일" className="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-1 focus:ring-[#16232A] focus:border-[#16232A] dark:focus:ring-[#FF5B04] dark:focus:border-[#FF5B04] bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-sm transition-all" /></div>
                        <div className="space-y-1.5"><label className="text-[13px] font-bold text-slate-700 dark:text-slate-300 block">{t('auth.password')}<span className="text-red-500">*</span></label><div className="relative"><input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="비밀번호" className="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-1 focus:ring-[#16232A] focus:border-[#16232A] dark:focus:ring-[#FF5B04] dark:focus:border-[#FF5B04] bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-sm transition-all" /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 hover:text-[#16232A] dark:hover:text-[#FF5B04] transition-colors">{showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button></div></div>
                        <div className="flex items-start gap-3 pt-2"><label className="flex items-start gap-3 cursor-pointer group w-full"><div className="relative flex items-center mt-1"><input type="checkbox" className="peer appearance-none w-4 h-4 rounded border border-slate-300 dark:border-slate-600 dark:bg-slate-800 checked:bg-[#16232A] checked:border-[#16232A] dark:checked:bg-[#FF5B04] dark:checked:border-[#FF5B04] transition-all cursor-pointer shrink-0" /><svg className="absolute w-2.5 h-2.5 ml-[-1px] text-white opacity-0 peer-checked:opacity-100 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="4"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg></div><span className="text-[13px] font-medium text-slate-500 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-300 transition-colors leading-[1.6]">계정을 생성하면 <span className="text-slate-700 dark:text-slate-200 font-bold">이용약관</span> 및 <span className="text-slate-700 dark:text-slate-200 font-bold">개인정보 처리방침</span>에 동의하는 것으로 간주됩니다.</span></label></div>
                        <button type="submit" className="w-full py-3.5 bg-[#16232A] hover:bg-[#23343e] dark:bg-[#FF5B04] dark:hover:bg-[#e04f00] text-white font-bold text-sm rounded-lg transition-colors mt-2">{t('auth.signUp')}</button>
                    </form>
                    <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-6">{t('auth.alreadyHaveAccount')} <button onClick={() => navigate('/signin')} className="text-[#16232A] dark:text-[#FF5B04] font-medium hover:underline">{t('auth.signIn')}</button></p>
                </div>
            </div>
            <div className="hidden lg:flex w-1/2 bg-[#16232A] relative overflow-hidden flex-col items-center justify-center">
