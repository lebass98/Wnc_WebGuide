...
interface CalendarSectionWrapperProps {
  title: string;
  description: string;
  snippet: CodeSnippet;
...
const CalendarSectionWrapper: React.FC<CalendarSectionWrapperProps> = ({ title, description, snippet, children }) => {
  ...
  const handleCopyCode = () => {
    const codeToCopy = activeTab === 'code'
      ? (codeMode === 'react' ? snippet.react : (htmlSubTab === 'html' ? snippet.html : (htmlSubTab === 'css' ? snippet.css : snippet.js)))
      : snippet.fullHtml;
    ...
    <div className="space-y-4 font-sans mb-[40px]">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 bg-slate-50/50 dark:bg-slate-800/40 p-4 rounded-2xl">
        ...
        <div className="flex flex-wrap items-center gap-2 justify-end">
          ...
          {/* Device and Theme Selectors (Only for Preview Tab) */}
          {activeTab === 'preview' && (
            <>
              ...
              <div className="flex items-center p-1 bg-slate-100 dark:bg-slate-800/80 rounded-xl">
                <button
                  ...
                  className={`relative group p-1.5 rounded-lg transition-colors cursor-pointer ${device === 'desktop' ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                >
                ...
                <button
                  ...
                  className={`relative group p-1.5 rounded-lg transition-colors cursor-pointer ${device === 'tablet' ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                >
                ...
                <button
                  ...
                  className={`relative group p-1.5 rounded-lg transition-colors cursor-pointer ${device === 'mobile' ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                >
              ...
              {/* Theme Simulator (Only in HTML Preview mode) */}
              {previewMode === 'html' && (
                <div className="flex items-center p-1 bg-slate-100 dark:bg-slate-800/80 rounded-xl">
                  <button
                    onClick={() => setTheme('light')}
                    className={`relative group p-1.5 rounded-lg transition-colors cursor-pointer ${theme === 'light' ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                  ...
                  <button
                    ...
                    className={`relative group p-1.5 rounded-lg transition-colors cursor-pointer ${theme === 'dark' ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  >
          ...
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2.5 px-2 py-1 text-[10px] font-bold text-white bg-slate-900/90 dark:bg-slate-800/95 rounded-md shadow-md opacity-0 group-hover:opacity-100 transition-all duration-200 scale-95 group-hover:scale-100 pointer-events-none whitespace-nowrap z-50">
                {copied ? "복사 완료!" : (activeTab === 'code' ? "현재 선택된 코드 복사" : "HTML/CSS/JS 전체 소스 복사")}
                <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-slate-900/90 dark:border-t-slate-800/95" />
              </div>
            </button>
          </div>
        ...
        {activeTab === 'preview' ? (
          ...
          <div className="bg-[#0F172A] flex flex-col min-h-[350px]">
            ...
            <div className="p-6 overflow-auto max-h-[500px]">
              <pre className="text-slate-300 text-xs font-mono select-all whitespace-pre-wrap leading-relaxed">
                {codeMode === 'react' ? snippet.react : (htmlSubTab === 'html' ? snippet.html : (htmlSubTab === 'css' ? snippet.css : snippet.js))}
              </pre>
            </div>
          </div>
